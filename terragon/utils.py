...
def read_and_process_bands(img_folder, bands, gdf, target_resolution):
    ...
    if bands:
        band_files = [
            img_path
            for img_path in img_folder.glob(f"**/IMG_DATA/**/*.jp2")
            if any(band in img_path.stem for band in bands)
        ]
    else:
    ...
    for band_file in band_files:
        ...
        # Add time coordinate
        time = pd.to_datetime(
            re.search(r"\d{8}", str(band_file)).group(), format="%Y%m%d"
        )
    ...
    band_arrays = sorted(
        band_arrays, key=lambda da: da.name
    )  # Example: sort by band name
...
def build_minicube(output_dir, bands, gdf, resolution, num_workers=4):
    ...
    imgs_datasets = Parallel(n_jobs=num_workers)(
        delayed(read_and_process_bands)(img_dir, bands, gdf, resolution)
...
def resolve_resolution(shp, resolution):
    ...
    if crs.is_geographic:
        print(
            f"Assuming the provided resolution of '{resolution}' is in meters since the CRS is geographic. Automatically converting this resolution to degrees."
        )
...
def filter_unique_items(items, tile_id, product_type, max_cloud_cover=50):
    ...
    for item in items:
        ...
        level, cloud_cover = extract(item, "s2:product_type"), extract(
            item, "eo:cloud_cover", 100
        )
        key = (date, tile_id)
        ...
        if (
            ...
            or (
                level == product_type
                and extract(seen[key], "s2:product_type") != product_type
            )
            or (
                level == extract(seen[key], "s2:product_type")
                and cloud_cover < extract(seen[key], "eo:cloud_cover", 100)
            )
        ):
...
def stack_cdse_bands(
    img_folder: Path, shp: gpd.GeoDataFrame, target_res: int
) -> xr.Dataset:
    ...
    data_arrays = sorted(
        data_arrays, key=lambda da: da.name
    )  # Example: sort by band name
    return xr.merge(data_arrays)


def stack_asf_bands(
    img_folder: Path, shp: gpd.GeoDataFrame, resolution: int
) -> xr.Dataset:
    data_arrays = []
